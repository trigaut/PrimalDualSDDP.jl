...
function initialize_lift_primal!(m::JuMP.Model, dhm::DecisionHazardModel, t::Int, Vₜ₊₁::PolyhedralFunction)
    nξ = length(m[:xₜ₊₁])
    @variable(m, θ[1:nξ])
...
function primalsolve!(::DecisionHazardModel, m::JuMP.Model, xₜ::Vector{Float64})
...
function new_cut!(dhm::DecisionHazardModel, Vₜ::PolyhedralFunction, m::JuMP.Model, xₜ::Vector{Float64})
...
function update!(dhm::DecisionHazardModel, mₜ::JuMP.Model, Vₜ₊₁::PolyhedralFunction)
    nξ = length(mₜ[:θ])
    for i in 1:nξ
