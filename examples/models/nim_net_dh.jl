...
function NonIslandedNetModel(
    ...
    function fₜ(t, xₜ, uₜ, ξₜ₊₁)
        xₜ₊₁ = [xₜ[1] + ρc * uₜ[1] - 1 / ρd * uₜ[2], xₜ[2] - uₜ[1] - uₜ[2], α[t] * xₜ[3] + β[t] + ξₜ₊₁[1]]
        return xₜ₊₁
    end

    return NonIslandedNetModel(
        Δt,
        capacity,
        ρc,
        ρd,
...
function PrimalDualSDDP.bellman_operator(nim::NonIslandedNetModel, t::Int)
    ...
    @constraint(m, socₜ + nim.ρc * uₜ⁺ - 1 / nim.ρd * uₜ⁻ >= 0.0)
...
function PrimalDualSDDP.dual_bellman_operator(nim::NonIslandedNetModel, t::Int, l1_regularization::Real)
