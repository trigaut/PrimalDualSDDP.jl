...
const wdm = WaterDamModel(Δt, cap, umax, csell, rain_scenarios, bins)

const primal_pruner = PrimalDualSDDP.ExactPruner(SOLVER, lb = [0.0], ub = [cap])

V = [PrimalDualSDDP.PolyhedralFunction([0.0]', [-1e4]) for t in 1:T]
push!(V, PrimalDualSDDP.PolyhedralFunction([0.0]', [0.0]))
x₀s = [(0.0,)]
primal_models = @time PrimalDualSDDP.primalsddp!(wdm, V, 100, x₀s, pruner = primal_pruner, nprune = 200);
...
dual_models = @time PrimalDualSDDP.dualsddp!(
    wdm,
    D,
    100,
    λ₀s,
